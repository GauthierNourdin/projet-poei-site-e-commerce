...
public class InscriptionVendeurServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (complementAdresse == null)
			complementAdresse = "";
		if (testValidite) {
			Vendeur vendeur = new Vendeur(nom, prenom, adresseMail, numeroTelephone, identifiantConnexion, motDePasse,
			...
			try {
				...
				this.getServletContext().getRequestDispatcher("/WEB-INF/vendeur/inscription.jsp").forward(request,
						response);
				return;
			}
			if (vendeur == null) {
				...
				request.setAttribute("erreurinscription",
						"Erreur : absence de sauvagarde du vendeur dans la base de données");
	...
	protected boolean testIdentifiantConnexion(String identifiantConnexion) {
		...
		if (identifiantConnexion != null && identifiantConnexion.length() > 3
				&& vendeurService.findByIdentifiantConnexion(identifiantConnexion) == null)
			return true;
		return false;
	}
