...
public class TestPanierService {
	...
	public static void main(String[] args) {
		...
		if(!panierService.save(panier1)) System.out.println("Echec de l'ajout : produit identique déjà présent");
		System.out.println(panierService);
		
		if(!panierService.save(panier2)) System.out.println("Echec de l'ajout : produit identique déjà présent");
		System.out.println(panierService);
		
		if(!panierService.save(panier3)) System.out.println("Echec de l'ajout : produit identique déjà présent");
		System.out.println(panierService);
		
		if(!panierService.save(panier2)) System.out.println("Echec de l'ajout : produit identique déjà présent");

		if(!panierService.remove(panier1)) System.out.println("Echec de la suppresion du produit : ce produit n'est pas présent dans la liste");
		System.out.println(panierService);
		
		if(!panierService.remove(panier4)) System.out.println("Echec de la suppresion du produit : ce produit n'est pas présent dans la liste");
		
		Panier panier5 = new Panier("00000003", client4);
		if(!panierService.update(panier5)) System.out.println("Echec de l'update du produit : ce produit n'est pas présent dans la liste");
		System.out.println(panierService);
		
		Panier panier6 = new Panier("00000004", client3);
		if(!panierService.update(panier6)) System.out.println("Echec de l'update du produit : ce produit n'est pas présent dans la liste");
		
		ArrayList<Panier> produitsAInstantT = panierService.findAll();
		System.out.println(produitsAInstantT);
		
		System.out.println("Le produit ayant pour id : 00000002 est : " + panierService.findById("00000002"));
		System.out.println("Le produit ayant pour id : 00000004 est : " + panierService.findById("00000004"));
